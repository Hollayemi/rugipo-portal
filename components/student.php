 ...
 * Let's add some new cndidates with a new energy. Following is the simple form for adding a new student record to the previous existing records.
 <form class="shadow p-3" action="" method="post">
  ...
  <div class="form-group">
    <label for="c-roll">Matric Number :- </label>
    <input type="text" name="c-roll" class="form-control" id="c-roll" aria-describedby="c-roll" placeholder="Enter Student Matric Number in numeric value only. Ex :- ND/CSC/32352" required>
  </div>
  <div class="form-group">
  ...
  <div class="form-group">
    <label for="c-post">Student Post </label>
 
  <select class="custom-select" name="c-post" id="c-post" required>
    <option value="">Choose...</option>
    
      <option value="Department President">Department President</option>
      <option value="Class Rep">Class Rep</option>
      <option value="Class Rep Assistant">Class Rep Assistant</option>
      <option value="Department Gen Sec">Department Gen Sec</option>
      <option value="Department Treasurer">Department Treasurer</option>
...
// add new class...
    ...
    $ck2 = "select * from `students` where roll_number='$croll'";
		...
		$query1 = "INSERT INTO `students` (`batch_id`,`email`,`mobile`,`roll_number`,`name`,`post`, `date`) VALUES ('$cbatch','$cemail','$cmobile','$croll','$cname','$cpost',NOW())";
